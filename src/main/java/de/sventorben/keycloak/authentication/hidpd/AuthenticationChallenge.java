...
final class AuthenticationChallenge {
    ...
    AuthenticationChallenge(AuthenticationFlowContext context, RememberMe rememberMe, LoginHint loginHint, LoginForm loginForm, HomeIdpDiscoveryConfig config, Reauthentication reauthentication) {
    ...
    void forceChallenge() {
        ...
        if (reauthentication.required() && context.getUser() != null) {
            formData.add(AuthenticationManager.FORM_USERNAME, context.getUser().getFirstAttribute(config.userAttribute()));
        } else {
