...
| Forward users with no email        | `forwardNoEmail`    | `boolean` | Requires `forwardToLinkedIdp` to be true. If switched on (`true`), users without an email address be forwarded to the first IdP that is linked to that user.                                                                                            |
