...
| Forward users with no email        | `forwardNoEmail`    | `boolean` | If switched on (`true`), users without an email address will be forwarded to the first IdP that is linked to that user. This bypasses email domain matching because no domain is available.<br><br>If switched off (`false`, default), users without an email address will not be forwarded to a linked IdP and will continue with the normal authentication flow.<br><br>This option requires `forwardToLinkedIdp` to be switched on (`true`). It is only applied when no email address can be determined for the user.                                                                                       |
